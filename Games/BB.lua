...
local AimbotTarget, SilentAimTarget, Aimbot, Toroiseshell
 = nil, nil, false, require(ReplicatedStorage.TS)
...
local Window = Parvus.Utilities.UI:Window({Name = "Parvus Hub",Enabled = Parvus.Config.UI.Enabled,Color = Parvus.Utilities.Config:TableToColor(Parvus.Config.UI.Color),Position = UDim2.new(0.2,-248,0.5,-248)}) do
    local AimAssistTab = Window:Tab({Name = "Aim Assist"}) do
        local AimbotSection = AimAssistTab:Section({Name = "Aimbot",Side = "Left"}) do
            ...
            AimbotSection:Dropdown({Name = "Priority",Default = Parvus.Config.AimAssist.Aimbot.Priority,List = {"Head","Neck","Chest","Abdomen","Hips"},Callback = function(String)
        ...
        local SilentAimSection = AimAssistTab:Section({Name = "Silent Aim",Side = "Right"}) do
            ...
            SilentAimSection:Dropdown({Name = "Priority",Default = Parvus.Config.AimAssist.SilentAim.Priority,List = {"Head","Neck","Chest","Abdomen","Hips"},Callback = function(String)
...
local function TeamCheck(Target)
    return (Parvus.Config.AimAssist.TeamCheck and LocalPlayer.Team ~= Target.Team) or
    (Parvus.Config.AimAssist.TeamCheck and tostring(Target.Team) == "FFA") or true
end
...
local function GetTarget(FoV,Priority,Visibility)
    ...
    for Index, Target in pairs(PlayerService:GetPlayers()) do
        ...
        if Target ~= LocalPlayer and Hitbox and TeamCheck(Target) then
            local ScreenPosition, OnScreen = Camera:WorldToViewportPoint(Hitbox.Position)
            local Magnitude = (Vector2.new(ScreenPosition.X, ScreenPosition.Y) - UserInputService:GetMouseLocation()).Magnitude
            if OnScreen and WallCheck(Visibility,Hitbox,Target.Character) and FieldOfView > Magnitude then
...
Toroiseshell.Network.Fire = function(self, ...)
    ...
    if Parvus.Config.AimAssist.SilentAim.Enabled and SilentAimTarget then
        if args[2] == "__Hit" and math.random(0,100) <= Parvus.Config.AimAssist.SilentAim.HitChance then
            args[4] = SilentAimTarget.Position
            args[5] = SilentAimTarget
        end
    end
